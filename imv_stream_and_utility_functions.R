...
## Modified ACE function, from Tsphere R package
## included a few extra "drop = FALSE" to fix case q = 2
ACE <- function (x, sig, delt, sigi, delti, M, thr = 1e-04, maxit = 1000) {
  n = nrow(x)
  p = ncol(x)
  xhat = x
  ...
  while (ind > thr & iter < maxit) {
    ...
    for (i in rmi) {
      ...
      if (nrmi[i] >= round(p/2)) {
        ...
        xhat[i, mj] = ey[mj] + swpz[mj, -mj, drop = FALSE] %*% 
          (xhat[i, -mj] - ey[-mj])
      }
      else {
    ...
    for (j in cmj) {
      ey = M[, j] + (xhat[, -j, drop = FALSE] - M[, -j, drop = FALSE]) %*% (-delti[-j, j, drop = FALSE]/delti[j, j])
      covy = sig/delti[j, j]
      mi = (1:n)[is.na(x[, j])]
      if (ncmj[j] >= round(n/2)) {
        swpz = matinv(covy, (1:n)[-mi])
        xhat[mi, j] = ey[mi] + swpz[mi, -mi, drop = FALSE] %*% 
          (xhat[-mi, j] - ey[-mi])
      }
      else {
